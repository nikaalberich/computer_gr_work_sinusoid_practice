...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        // private double Sinsoidic(double x, double a, double b, double c) => (a * (Math.Pow(x, 2)) + (b * x) + c);

        private double Sinsoidic(double x, double a) => (x*Math.Sin(a*x));

        private double FindDelta(double x1, double x2) => (x2 - x1) / 1000;
   
        private double findMaximumOfY(double x1, double x2, double a, double maxy, double dt)
        {
            for (double x = x1; x < x2; x += dt)
            {
                double y = Sinsoidic(x, a);
                if (y > maxy)
                {
                    maxy = y;
                }
        ...
        private double findMinimumOfY(double x1, double x2, double a, double miny, double dt)
        {
            for (double x = x1; x < x2; x += dt)
            {
                double y = Sinsoidic(x, a);
        ...
        private void drawParabolicFunction(double x1, double x2, double a, double miny, double dt, double maxy, double minx, double maxx, PaintEventArgs e, Pen penForLine)
        {
            ...
            for (double x = x1; x < x2; x += dt)
            {
                ...
                double y1 = Sinsoidic(x, a );
                double y2 = Sinsoidic(x + dt, a );

                double xu1 = Sinsoidic(x, a);
                double xu2 = Sinsoidic(x + dt, a);
        ...
        private void PictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            double miny = Sinsoidic(x1, a);
            double maxy = miny;

            double minx = Sinsoidic(x1, a);
            double maxx = minx;

            maxy = findMaximumOfY(x1, x2, a, maxy, dt);
            miny = findMinimumOfY(x1, x2, a, miny, dt);
            ...
            drawParabolicFunction(x1, x2, a,  miny, dt, maxy, minx, maxx, e, penForLine);
        }
